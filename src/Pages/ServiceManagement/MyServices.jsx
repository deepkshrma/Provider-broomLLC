...
export default function MyServices() {
  ...
  const fetchMyServices = async () => {
    try {
      ...
      // Get subscribed categories from profile
      ...
      const catsRes = await axios.get(
        `${BASE_URL}/admin/get-frontend-categories`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (catsRes.data.status) {
        const filteredParents = catsRes.data.data
          .map((parent) => ({
            ...parent,
        ...
        setFilteredParentCategories(filteredParents);
        setSubCategories(filteredParents.flatMap((p) => p.subcategories));
      }
    } catch (err) {
  ...
  const handleParentClick = (parentId) => {
    ...
    if (parentId === "all") {
      setSubCategories(
        filteredParentCategories.flatMap((p) =>
          p.subcategories.filter((sub) =>
            myCategories.some((my) => my.id === sub._id)
          )
        )
      );
    } else {
      const parent = filteredParentCategories.find((p) => p._id === parentId);
      setSubCategories(
        parent
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center px-3 bg-blue-50 py-2 mt-auto">
                      <a href="#" className="text-[12px] text-blue-500">
                        <u>
                          <b className="mr-1">{sub.serviceCount || 0}</b>
                          Service
                        </u>
                      </a>
                      <div
                        className="w-1/2 text-[12px] text-white bg-yellow-500 text-center py-2 rounded cursor-pointer"
                        onClick={() => handleSubscriptionToggle(sub._id)}
                      >
                        UNSUBSCRIBE
                      </div>
                    </div>
